...
from .modify_llama_oursk import LlamaAttention_heavy_hitter_our_sketch
...
def convert_kvcache_llama_heavy_recent(model, config):
    ...
    for name, module in reversed(model._modules.items()):
        ...
        if isinstance(module, LlamaAttention):
            global attn_layer_cnt
            attn_layer_cnt = attn_layer_cnt + 1
            config.layer_idx = config.num_hidden_layers - attn_layer_cnt
        
            if config.hh_method == "h2o":
                model._modules[name] = LlamaAttention_heavy_hitter(config)
            elif config.hh_method == "ours":
                config = get_our_config(config)
                model._modules[name] = LlamaAttention_heavy_hitter_our_sketch(config)                                                                                              
            else:
                print("no such method "+config.hh_method)
                exit()
...
# pyramid alloc
def get_our_config(config):
    config.notcompress = False
    config.heavy_ratio_layers = []
    config.recent_ratio_layers = []

    total_ratio_acrosslayer = (config.heavy_ratio + config.recent_ratio) * config.num_hidden_layers
    pyra_ratio_acrosslayer = (config.pyramidinfer_recent_ratio + 
                             (config.pyramidinfer_decay_ratio + 1.0) / 2 * (1 - config.pyramidinfer_recent_ratio)) * config.num_hidden_layers
    scale_ratio = total_ratio_acrosslayer / pyra_ratio_acrosslayer
    for idx in range(config.num_hidden_layers):
        config.recent_ratio_layers.append(config.pyramidinfer_recent_ratio * scale_ratio)

        if config.pyramidinfer_decay_strategy == "linear":
            schedule_prefill_decay_ratio = (1.0 - config.pyramidinfer_decay_ratio) * (idx / config.num_hidden_layers) + config.pyramidinfer_decay_ratio
        if config.pyramidinfer_decay_strategy == "cosine":
            schedule_prefill_decay_ratio = (1.0 - config.pyramidinfer_decay_ratio) * (math.cos(math.pi * idx / config.num_hidden_layers) + 1) / 2\
                                                + config.pyramidinfer_decay_ratio
        else:
            schedule_prefill_decay_ratio = (config.pyramidinfer_decay_ratio + 1.0) / 2
        
        schedule_prefill_decay_ratio *= (1 - config.pyramidinfer_recent_ratio)
        config.heavy_ratio_layers.append(schedule_prefill_decay_ratio * scale_ratio)
